    ...
    <div>
      {loading && (
        <div className="grid grid-cols-2 gap-2">
          <Skeleton variant="box" className="h-16 rounded-xl" />
          <Skeleton variant="box" className="h-16 rounded-xl" />
          <Skeleton variant="box" className="h-16 rounded-xl" />
          <Skeleton variant="box" className="h-16 rounded-xl" />
        </div>
      ...
      {!loading && shots.length > 0 && (
        <div className="grid grid-cols-3 gap-2 max-h-44 overflow-y-auto">
          {shots.map((s, i) => (
            <div
              key={s.name}
              className="group relative glass rounded-xl overflow-hidden hover:scale-[1.03] hover:shadow-lg hover:shadow-black/30 transition-all duration-200 cursor-pointer motion-safe:animate-fade-in"
            ...
            >
              ...
              <button
                ...
                className="absolute top-1 right-1 p-1 rounded-lg bg-black/60 backdrop-blur-sm text-gray-400 hover:text-red-400 transition-all opacity-0 group-hover:opacity-100"
              >
        ...
        {lightbox !== null && shots[lightbox] && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-400 font-mono">{parseName(shots[lightbox].name)}</span>
              <div className="flex items-center gap-1.5">
                ...
                <a
                  ...
                  className="p-1.5 text-gray-500 hover:text-accent-light transition-colors rounded-lg hover:bg-white/5"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <button
                  ...
                  className="p-1.5 text-gray-500 hover:text-danger transition-colors rounded-lg hover:bg-white/5"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-danger hover:bg-danger/90 text-white font-medium transition-all text-sm shadow-lg shadow-danger/20"
            >
              Delete
            </button>
          </div>
